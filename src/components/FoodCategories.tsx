...
const FoodCategories:React.FC<FoodCategoriesProps> = ({data}) => {
    ...
    <Container className="py-10">
      ...
      <div className="flex flex-col md:flex-row gap-5 h-[700px] md:h-[23rem] w-full my-10">
        {data.map((item, index) => (
          ...
          >
            <Image
              src={item.categoryImage ?? '/assets/images/logo.png'}
            ...
            <div className={`relative z-10 flex items-end h-full p-2 ease-in-out duration-150  bg-gradient-to-b to-customBlack/80 justify-between
             ${active === index ? 'from-white/0' : 'from-customBlack/20'}`}>
              <h4 className="text-2xl md:text-3xl lora-heading capitalize vertical-text">{item.categoryName}</h4>
              <button className={`${active === index ? '' : 'hidden'}
               text-sm flex items-center gap-2 border-b `} onClick={()=>router.push(`/menu#${item.categoryName.toLowerCase()}`)}>
